...
async def _extract_one(
    ...
    *,
    ...
    # Guided JSON cannot wrap a <thinking> prefix — disable when CoT is on.
...
async def _extract_one_multipass(
    ...
    *,
    ...
    scout_max = min(12_000, 1200 + 45 * ncols)
    doc_key = str(doc_meta.get("doc_id") or doc_meta.get("filing_id") or "")
...
async def _run_extraction(
    ...
    p2d = load_prompt2dataset_config()
    json_schema = build_dynamic_json_schema(
        columns,
        with_evidence=True,
...
def extract_one_filing(
    ...
    """
    cfg = get_settings()
    columns: list[SchemaColumn] = state.get("proposed_columns", [])
    if not columns:
        return {}
