...
def _apply_repo_vllm_env_overrides(root: Path) -> None:
    ...
    ``VLLM_MAX_TOKENS`` / ``VLLM_MODEL_MAX_CONTEXT_TOKENS`` are not forced here so a
    per-invocation export can override ``.env`` for wide schema extraction.
    """
    try:
    ...
    for key in (
        ...
        "VLLM_MODEL_NAME",
        "VLLM_TIMEOUT_SEC",
        # Context/output caps: do not force from .env here — operators override per run
        # (e.g. wide codebooks). Values still load via load_dotenv(override=False) first.
